...
public interface MatchesRepository extends JpaRepository<Matches, String> {
    @Query("SELECT COUNT(m) FROM Matches m JOIN Players p ON m.checksum = p.matchChecksum WHERE p.name IN ('Nene') AND p.teamName = m.winnerName AND m.analyzeDate >= :startDate AND m.analyzeDate <= :endDate" )
    public Integer findWins(Date startDate, Date endDate);

    @Query("SELECT COUNT(m) FROM Matches m JOIN Players p ON m.checksum = p.matchChecksum WHERE p.name IN ('Nene') AND p.teamName != m.winnerName AND m.analyzeDate >= :startDate AND m.analyzeDate <= :endDate" )
    public Integer findLoses(Date startDate, Date endDate);

    @Query("SELECT COUNT(m) FROM Matches m WHERE m.winnerName is NULL AND m.analyzeDate >= :startDate AND m.analyzeDate <= :endDate" )
    public Integer findTies(Date startDate, Date endDate);

    @Query(value ="SELECT m.checksum as checksum, " +
           "m.analyze_date as date, " +
           "d.map_name as map, " +
           ...
           "d.map_name as demoFileName " +
           "FROM Matches m JOIN Teams t ON m.checksum = t.match_checksum JOIN Players p ON m.checksum = p.match_checksum JOIN Demos d ON m.checksum = d.checksum " +
           "WHERE p.name IN ('Nene') AND date >= :startDate AND date <= :endDate " +
           "GROUP BY m.checksum, m.analyze_date, d.map_name, p.team_name, m.winner_name, m.winner_side, m.overtime_count, d.duration ORDER BY m.analyze_date DESC", nativeQuery = true)
    ...
    @Query(value="SELECT COUNT(*) " +
                "FROM (select " +
                    ...
                    "FROM (select " +
                        ...
                        "p.score, " + 
                        "m.analyze_date " +
                        "from players p join matches m on p.match_checksum = m.checksum join demos d on m.checksum = d.checksum " +
                        "where p.\"name\" in ('ShinChan', 'Kazama', 'Nene', 'SwagChan', 'The Mafios') AND m.analyze_date >= :startDate AND m.analyze_date <= :endDate) t1) t2 " +
