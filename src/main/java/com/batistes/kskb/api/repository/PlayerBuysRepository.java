...
public interface PlayerBuysRepository extends JpaRepository<PlayerBuys, Long> {
    ...
    @Query("SELECT pb FROM PlayerBuys pb JOIN Matches m ON pb.matchChecksum = m.checksum WHERE m.analyzeDate BETWEEN :startDate AND :endDate AND pb.playerName IN (:players) AND weaponType = 'grenade'")
    ...
    @Query("SELECT new com.batistes.kskb.api.dto.StatisticDTO(pb.playerName, COUNT(pb)) FROM PlayerBuys pb JOIN Matches m ON pb.matchChecksum = m.checksum " +
            "WHERE m.analyzeDate BETWEEN :startDate AND :endDate AND (pb.playerName IN :players) AND pb.weaponType = 'grenade'" +
