...
public interface BombsPlantStartRepository extends JpaRepository<BombsPlantStart, Long> {
    @Query("SELECT bp FROM BombsPlantStart bp JOIN Matches m ON bp.matchChecksum = m.checksum WHERE m.analyzeDate BETWEEN :startDate AND :endDate AND (bp.planterName IN :players)")
